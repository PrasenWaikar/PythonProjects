...
while True:
    ...
    for(x,y,w,h) in faces:
        ...
        exist=os.path.isfile("Attendance/Attendance1_" + date +".csv")
        #insert a rectangle
    ...
    if k== ord('o') and not attendance_taken and not attendance_recorded:
        ...
        if exist:
            with open("Attendance/Attendance1_" +  date + ".csv", "a") as csvfile:
            ...
            with open(r"Attendance/Attendance1_" + date + ".csv","w+")as csvFile:
                writer=csv.writer(csvFile)
...
if first_in is not None and last_out is not None:
    with open("Attendance/Attendance1_" + date + ".csv", "a") as csvfile:
        writer = csv.writer(csvfile)
        first_out_row = [str(output[0]) + " " + date + " " + first_in]
        last_in_row = [str(output[0]) + " " + date + " " + last_out]
